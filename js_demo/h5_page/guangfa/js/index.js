...
window.onload = window.onresize = function(){
    pageResponse({
        selectors : '.index_page,page_content',     //模块选择器，使用querySelectorAll的方法
...
document.addEventListener("DOMContentLoaded", function() {
    pageResponse({
        selectors: '.index_page,.page_content',     //模块的类名，使用class来控制页面上的模块(1个或多个)
...
$(function(){
   ...
   //预加载图片等资源
    ...
    $(".catch_btn").click(function(){
        TweenMax.to(".title", 1, {opacity:0,top:"10%"});
        TweenMax.to(".catch_btn", 1, {opacity:0,scale:0.9});
        TweenMax.to(".index_page", 1, {opacity:0,scale:0.9,onComplete:function(){
            $(".index_page").hide();
            $(".page_content").show();
        }}).delay(1);
    ...
    function p2_show(){
        ...
        function loaddata(j){
            if(j>=0&&j<=5){
            $("img").attr("src","../guangfa/img/Q"+(j+1)+".png");

            for(var i=0;i<=3;i++){
                $($(".list").find("li")[i]).empty();
                $(".list").find("li")[i].append(txt[j][i]);
            }
        }
        }
            loaddata(0);
